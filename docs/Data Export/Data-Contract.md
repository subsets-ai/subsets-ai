...
- **Unchanged objects are not rewritten.** A retried or repeated run skips any object whose content is already in the bucket byte-for-byte, so bucket notifications fire on real changes only. In particular `_SUCCESS` finalizes when its partition first completes and is not rewritten by a later same-day correction - trigger reprocessing off the data file object, not the marker, if you need to catch corrections.
